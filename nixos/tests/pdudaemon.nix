...
{
  name = "PDUDaemon";
  meta.maintainers = with pkgs.lib.maintainers; [ aiyion ];
  ...
  testScript =
    ...
    ''
      with subtest("Wait for pdudaemon startup"):
          ...
          out = pdudaemonhost.succeed("systemctl cat pdudaemon.service")
          print(out)
          out = pdudaemonhost.succeed("curl 'http://localhost:16421/power/control/on?hostname=testpduhost&port=1'")
